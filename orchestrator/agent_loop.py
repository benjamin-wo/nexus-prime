...
def _build_system_prompt(is_admin: bool, timezone_name: str) -> str:
    ...
    return (
        ...
        "- Mailbox connection (Gmail/Outlook) is ALWAYS supported. To connect, link, log in, "
        "or re-authorize an email account -- or whenever any email task fails on a missing or "
        "broken mailbox -- call get_email_connection_status THIS TURN and relay its links "
        "verbatim. NEVER claim an email provider is 'not wired up', 'not connectable', or "
        "unsupported -- not even if earlier turns in this conversation said so -- and NEVER "
        "log a capability-gap request for mailbox connection: the tool lists a sign-in or "
        "re-authorize link for every configured provider.\n"
